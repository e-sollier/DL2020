...
for n_hidden_GNN in 8; do
	bsub -W 1:00 -R "rusage[mem=8000]" "python run.py -o out_alpha --classifier $classifier --n_hidden_GNN $n_hidden_GNN --n_obs_train $n_obs_train --infer_graph $infer_graph --seed $seed --alpha $alpha"
done
done
