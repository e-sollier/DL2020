...
class Dataset():
    ...
    def create_graph(self, method='glasso', **kwargs):
        #TODO: add **kwargs
        if method == 'glasso':
            self.Ah_train = glasso(self.X_train, **kwargs)
            self.Ah_test  = glasso(self.X_test, **kwargs)
    ...
    def score_graphs_eigv(self):
        # TODO: add error trap
        return compare_graphs_eigv(self.A_train, self.Ah_train), \
            compare_graphs_eigv(self.A_test, self.Ah_test)
